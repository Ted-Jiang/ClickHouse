...
function (build_clang_builtin target_triple OUT_VARIABLE)
    ...
    if (target_triple STREQUAL "x86_64-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-x86_64")
    ...
    elseif (target_triple STREQUAL "aarch64-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-aarch64")
    ...
    elseif (target_triple STREQUAL "loongarch64-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-loongarch64")
    ...
    elseif (target_triple STREQUAL "powerpc64le-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-powerpc64le")
    ...
    elseif (target_triple STREQUAL "riscv64-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-riscv64")
    ...
    elseif (target_triple STREQUAL "s390x-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-s390x")
    ...
    elseif (target_triple STREQUAL "e2k-linux-gnu")
        ...
        set (BUILTINS_CMAKE_TARGET "clang_rt.builtins-e2k")
    ...
    foreach (LIB IN ITEMS ${EXTRA_BUILTINS_LIBRARIES})
        # Get the architecture part from BUILTINS_CMAKE_TARGET (e.g., "x86_64" from "clang_rt.builtins-x86_64")
        string(REGEX REPLACE "clang_rt\\.builtins-(.+)" "\\1" ARCH "${BUILTINS_CMAKE_TARGET}")
        # Create CMAKE target name for the extra library (e.g., "clang_rt.xray-x86_64")
        set (NEW_CMAKE_TARGET "clang_rt.${LIB}-${ARCH}")
        # Create file path for the extra library
        string(REPLACE "builtins" "${LIB}" NEW_TARGET "${BUILTINS_TARGET}")
        list (APPEND EXTRA_TARGETS "--target" "${NEW_CMAKE_TARGET}")
    ...
    if (NEED_BUILTIN_BUILD)
        ...
        execute_process(
                COMMAND ${CMAKE_COMMAND} --build ${BUILTINS_BINARY_DIR} --target ${BUILTINS_CMAKE_TARGET} ${EXTRA_TARGETS}
                COMMAND_ECHO STDOUT
                COMMAND_ERROR_IS_FATAL ANY
        )
    endif ()
